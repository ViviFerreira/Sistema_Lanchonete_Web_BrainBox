    ...
    <body>
        ...
        if (($resultado_categoria) AND ( $resultado_categoria->num_rows != 0)) {
            ...
            while ($row_categoria = mysqli_fetch_assoc($resultado_categoria)) {
                ?>
                                                        <li><a><?php echo $row_categoria['tipo_categoria']; ?></a></li>
                <?php
            }
            ?>
                                </ul>
            ...
            echo "<div class='alert alert-danger' role='alert' style='margin-top:1em;'>Nenhuma categoria encontrada</div>";
        }
        ?>--->
        <div class="produtos">
            <form method="post">
                ...
                if (($resultado_categorias) AND ( $resultado_categorias->num_rows != 0)) {
                    ?>
                    <?php
                    while ($row_categorias = mysqli_fetch_assoc($resultado_categorias)) {
                        $categoria = $row_categorias['tipo_categoria'];
                        ?>
                        <div class="box_fora"> 
                            <label><?php echo $categoria; ?></label><br>
                            <?php
                            $result_produtos = "SELECT * FROM produto WHERE categoria='$categoria'";
                            $resultado_produtos = mysqli_query($conn, $result_produtos);
                            if ($resultado_produtos->num_rows != 0) {
                                while ($rows_produtos = mysqli_fetch_assoc($resultado_produtos)) {
                                    ?>
                                    <div class="box_produto">
                                        <p><?php echo $rows_produtos['nome_produto']; ?></p>
                                        <p>Preço: <?php echo $rows_produtos['preco_produto']; ?></p>
                                        <input type="checkbox">
                                    </div>
                                    <?php
                                }
                            } else {
                                echo "<div class='alert alert-danger' role='alert' style='margin-top:1em; width:60%;'>Nenhuma produto nessa categoria</div>";
                            }
                            ?>
                        </div>
                        <?php
